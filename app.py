...
def update_cost():
    if request.method == "POST":
        ...
        cursor.execute("INSERT INTO updated_cost (vehicle_number, updated_cost) VALUES (?, ?) ON CONFLICT(vehicle_number) DO UPDATE SET updated_cost=?",
                       (vehicle_number, updated_cost, updated_cost))
...
def total_cost():
    ...
    # Compute grand total separately
    cursor.execute("SELECT SUM(showroom_cost + updated_cost) FROM total_cost")
    total_cost_sum = cursor.fetchone()[0] or 0

    conn.close()

    return render_template("total_cost.html", total_cost_data=total_cost_data, total_cost=total_cost_sum)
